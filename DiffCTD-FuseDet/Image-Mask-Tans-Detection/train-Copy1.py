...
parser.add_argument("--dataset_names", default=['IRSTD-1K', 'NUDT-SIRST', 'NUAA-SIRST'], nargs='+', type=list)
...
def train():
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        ...
        if (idx_epoch + 1) % opt.every_save_pth == 0:
            save_pth = opt.save + '/' + opt.dataset_name + '/' +  str(idx_epoch + 1) + '.pth.tar'
            save_checkpoint({
                'epoch': idx_epoch + 1,
        ...
        if results1[1] > best_mIOU[1]:
            ...
            save_pth = opt.save + '/' + opt.dataset_name + '/' + 'best' + '.pth.tar'
            save_checkpoint({
                'epoch': idx_epoch + 1,
                'state_dict': net.state_dict(),
                'total_loss': total_loss_list,
        ...
        if (idx_epoch + 1) == opt.nEpochs and (idx_epoch + 1) % opt.every_save_pth != 0:
            save_pth = opt.save + '/' + opt.dataset_name + '/' +  'last_epoch' + '.pth.tar'
            save_checkpoint({
                'epoch': idx_epoch + 1,
                'state_dict': net.state_dict(),
                'total_loss': total_loss_list,
...
def test(save_pth):
    test_set = ValSetLoader(opt.dataset_dir, opt.dataset_name, opt.dataset_name, img_norm_cfg=opt.img_norm_cfg)
    test_loader = DataLoader(dataset=test_set, num_workers=1, batch_size=1, shuffle=False)
...
if __name__ == '__main__':
    ...
    for dataset_name in opt.dataset_names:
        ...
        for model_name in opt.model_names:
            ...
            if not os.path.exists(opt.save + '/' + opt.dataset_name):
                os.makedirs(opt.save + '/' + opt.dataset_name)
            opt.f = open(opt.save + '/' + opt.dataset_name + '/train.txt', 'w')
