...
parser.add_argument("--dataset_names", default=['IRSTD-1K', 'NUDT-SIRST', 'NUAA-SIRST'], nargs='+')
...
parser.add_argument("--begin_test", default=500, type=int) # 500
...
parser.add_argument('--dataset', type=str, default='SIRST3', help='dataset')
parser.add_argument('--aug-method', type=str, default='no_aug', help='aug method')
parser.add_argument('--aug-ratio', type=str, default='aug-0.', help='aug ratio')
parser.add_argument('--train-repeat-num', type=str, default='train-0', help='run multiple times')
parser.add_argument('--aug-repeat-num', type=str, default='no_aug', help='run multiple times')
...
def train():
    ## dataset
    if opt.aug_ratio != 'aug-0.':
        train_set = TrainAugSetLoader(args=opt, dataset_dir=opt.dataset_dir, dataset_name=opt.dataset,
                                      patch_size=opt.patchSize, img_norm_cfg=opt.img_norm_cfg)
    else:
        train_set = TrainSetLoader(dataset_dir=opt.dataset_dir, dataset_name=opt.dataset, patch_size=opt.patchSize,
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        ...
        results1 = (0, 0)
        results2 = (0, 0)
        for idx_iter, (img, gt_mask, mask_set) in enumerate(train_loader):
            img, gt_mask, mask_set = Variable(img).cuda(), Variable(gt_mask).cuda(), Variable(mask_set).cuda()
            if img.shape[0] == 1:
                continue
            # print(mask_set.shape)
            # print(img.shape)
            preds = net.forward(img, mask_set)
        ...
        if (idx_epoch + 1) % opt.every_print == 0:
            ...
            logger_train.info(time.ctime()[4:-5] + ' Epoch---%d, total_loss---%f,\n'
        ...
        if (idx_epoch + 1) >= opt.begin_test and (idx_epoch + 1) % opt.every_test == 0:
            test_set = TestSetLoader(opt.dataset_dir, opt.dataset, opt.dataset, img_norm_cfg=opt.img_norm_cfg)
            ...
            with torch.no_grad():
                ...
                eval_PD_FA = PD_FA()
                test_loss = []
                for idx_iter, (img, gt_mask, mask_set, size, _) in enumerate(test_loader):
                    img, mask_set = Variable(img).cuda(), Variable(mask_set).cuda()
                    pred = net.forward(img, mask_set)
        ...
        if (idx_epoch + 1) % opt.every_save_pth == 0:
            save_pth = os.path.join(opt.save_pkl, str(idx_epoch + 1) + '.pth.tar')
            save_checkpoint({
                'epoch': idx_epoch + 1,
        ...
        if results1[1] > best_mIOU[1]:
            ...
            logger_train.info("the best model epoch \t" + str(idx_epoch + 1) + '\n')
            print("pixAcc, mIoU:\t" + str(best_mIOU))
            print("testloss:\t" + str(test_loss[-1]))
            print("PD, FA:\t" + str(best_Pd))

            logger_train.info("pixAcc, mIoU:\t" + str(best_mIOU) + '\n')
            logger_train.info("PD, FA:\t" + str(best_Pd) + '\n')
            save_pth = os.path.join(opt.save_pkl, str(idx_epoch + 1) + '.pth.tar')
            save_checkpoint({
                'epoch': idx_epoch + 1,
                'state_dict': net.state_dict(),
...
def test(save_pth):
    ...
    with torch.no_grad():
        ...
        logger_train.info("pixAcc, mIoU:\t" + str(results1) + '\n')
        logger_train.info("PD, FA:\t" + str(results2) + '\n')
...
class Net(nn.Module):
    ...
    def forward(self, img, mask_set):
        return self.model(img, mask_set)
...
if __name__ == '__main__':
    opt.model_name = opt.model_names[0]

    ## folders
    opt.save_folder = os.path.join('result', '%s' % opt.dataset, '%s' % opt.aug_ratio, '%s' % opt.aug_method,
                                   '%s' % opt.aug_repeat_num, '%s' % opt.train_repeat_num)
    os.makedirs(opt.save_folder, exist_ok=True)
    opt.save_pkl = os.path.join(opt.save_folder, 'checkpoint')
    os.makedirs(opt.save_pkl, exist_ok=True)
    ...
    def handle_exception(exc_type, exc_value, exc_traceback):
        logging.info("异常", exc_info=(exc_type, exc_value, exc_traceback))
