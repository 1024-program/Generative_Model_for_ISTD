...
def random_crop(img, mask, mask_set, patch_size, pos_prob=None):
    h, w = img.shape
    if min(h, w) < patch_size:
        img = np.pad(img, ((0, max(h, patch_size) - h), (0, max(w, patch_size) - w)),
                     mode='constant')  # 将不足 256的一边填充至256
        for i in range(len(mask_set)):
            mask_set[i] = np.pad(mask_set[i], ((0, max(h, patch_size) - h), (0, max(w, patch_size) - w)),
                     mode='constant')
    ...
    while 1:
        ...
        for i in range(len(mask_set)):
            mask_set[i] = mask_set[i][h_start:h_end, w_start:w_end]
    ...
    return img_patch, mask_patch, mask_set
