...
class Channel_Embeddings(nn.Module):
    def __init__(self, config, patchsize, img_size, in_channels):
        ...
        n_patches = (img_size[0] // patch_size[0]) * (img_size[1] // patch_size[1])  # 4 * 4 = 16
...
class Block_ViT(nn.Module):
    ...
    def forward(self, emb1, emb2, emb3, emb4, emb1_mask, emb2_mask, emb3_mask, emb4_mask):
        embcat = []
        org1 = emb1
        org2 = emb2
        org3 = emb3
...
class Encoder(nn.Module):
    ...
    def forward(self, emb1, emb2, emb3, emb4,  emb1_masks, emb2_masks, emb3_masks, emb4_masks):
        attn_weights = []
        # for layer_block in self.layer:
        for i in range(0, len(self.layer), 2):
            ## 第一个交叉注意力，之后自注意力
            # if i == 0:
            #     layer_block = self.layer[i]
            #     emb1, emb2, emb3, emb4, weights = layer_block(emb1, emb2, emb3, emb4, emb1_masks, emb2_masks,
            #                                                   emb3_masks, emb4_masks)
            #
            # else:
            #     layer_block = self.layer[i]
            #     emb1, emb2, emb3, emb4, weights = layer_block(emb1, emb2, emb3, emb4,  emb1, emb2, emb3, emb4)

            ## 全部交叉注意力
            layer_block = self.layer[i]
            emb1_masks, emb2_masks, emb3_masks, emb4_masks, weights = layer_block(emb1, emb2, emb3, emb4, emb1_masks, emb2_masks,
                                                          emb3_masks, emb4_masks)
            layer_block = self.layer[i+1]
            emb1, emb2, emb3, emb4, weights = layer_block(emb1_masks, emb2_masks, emb3_masks, emb4_masks, emb1, emb2,
                                                          emb3, emb4)
...
class ChannelTransformer(nn.Module):
    def __init__(self, config, vis, img_size, channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4]):
        ...
        channel_num_mask=channel_num

        ### 掩膜编码器，采用 掩膜之间独立的方式，顺序输入到编码器中 ####
        self.embeddings_1_mask = Channel_Embeddings(config, self.patchSize_1, img_size=img_size, in_channels=channel_num_mask[0] // 16)
        self.embeddings_2_mask = Channel_Embeddings(config, self.patchSize_2, img_size=img_size // 2,
                                               in_channels=channel_num_mask[1]// 16)
        self.embeddings_3_mask = Channel_Embeddings(config, self.patchSize_3, img_size=img_size // 4,
                                               in_channels=channel_num_mask[2]// 16)
        self.embeddings_4_mask = Channel_Embeddings(config, self.patchSize_4, img_size=img_size // 8,
                                               in_channels=channel_num_mask[3]// 16)

        # self.embeddings_1_mask = Channel_Embeddings(config, self.patchSize_1, img_size=img_size, in_channels=channel_num[0])
        # self.embeddings_2_mask = Channel_Embeddings(config, self.patchSize_2, img_size=img_size // 2,
    ...
    def forward(self, en1, en2, en3, en4, en1_mask, en2_mask, en3_mask, en4_mask):
        emb1 = self.embeddings_1(en1) # 32 8 8
        emb2 = self.embeddings_2(en2) # 64 8 8
        emb3 = self.embeddings_3(en3)
        emb4 = self.embeddings_4(en4)


        ### 掩膜编码器，采用 掩膜之间独立的方式，顺序输入到编码器中 ####
        ch1 = en1_mask.shape[1]
        emb1_masks = self.embeddings_1_mask(en1_mask[:,0:int(ch1/16),:,:])
        ch2 = en2_mask.shape[1]
        emb2_masks = self.embeddings_2_mask(en2_mask[:,0:int(ch2/16),:,:])
        ch3 = en3_mask.shape[1]
        emb3_masks = self.embeddings_3_mask(en3_mask[:,0:int(ch3/16),:,:])
        ch4 = en4_mask.shape[1]
        emb4_masks = self.embeddings_4_mask(en4_mask[:,0:int(ch4/16),:,:])
        for m_idx in range(1, 16):
            emb1_mask_si = self.embeddings_1_mask(en1_mask[:, (ch1//16)*m_idx:(ch1//16)*(m_idx+1), :, :])
            emb1_masks = torch.cat([emb1_masks, emb1_mask_si], dim=1)
            emb2_mask_si = self.embeddings_2_mask(en2_mask[:, (ch2//16)*m_idx:(ch2//16)*(m_idx+1), :, :])
            emb2_masks = torch.cat([emb2_masks, emb2_mask_si], dim=1)
            emb3_mask_si = self.embeddings_3_mask(en3_mask[:, (ch3//16)*m_idx:(ch3//16)*(m_idx+1), :, :])
            emb3_masks = torch.cat([emb3_masks, emb3_mask_si], dim=1)
            emb4_mask_si = self.embeddings_4_mask(en4_mask[:, (ch4//16)*m_idx:(ch4//16)*(m_idx+1), :, :])
            emb4_masks = torch.cat([emb4_masks, emb4_mask_si], dim=1)

        # emb1_masks = self.embeddings_1(en1_mask)  # 32 8 8
        # emb2_masks = self.embeddings_2(en2_mask)  # 64 8 8
        # emb3_masks = self.embeddings_3(en3_mask)
        # emb4_masks = self.embeddings_4(en4_mask)

        encoded1, encoded2, encoded3, encoded4, attn_weights = self.encoder(emb1, emb2, emb3, emb4, emb1_masks, emb2_masks, emb3_masks, emb4_masks)  # (B, n_patch, hidden)
...
class SCTransNet(nn.Module):
    def __init__(self, config, n_channels=1, n_classes=1, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.inc_mask = self._make_layer(block, n_channels, int(in_channels / 16))
        self.down_encoder1_mask = self._make_layer(block, int(in_channels / 16), int((in_channels * 2) / 16), 1)  # 64  128
        self.down_encoder2_mask = self._make_layer(block, int((in_channels * 2) / 16), int((in_channels * 4) / 16), 1)  # 64  128
        self.down_encoder3_mask = self._make_layer(block, int((in_channels * 4) / 16), int((in_channels * 8) / 16), 1)  # 64  128
        # self.down_encoder4_mask = self._make_layer(block, (in_channels * 8) / 16, (in_channels * 8) / 16, 1)  # 64  128


        # # ### 掩膜编码器，采用掩膜之间互相特征融合卷积的方式 ####
        # self.inc_mask = self._make_layer(block, 16, in_channels)
        # self.down_encoder1_mask = self._make_layer(block, int(in_channels ), int((in_channels * 2)), 1)  # 64  128
        # self.down_encoder2_mask = self._make_layer(block, int((in_channels * 2)), int((in_channels * 4)), 1)  # 64  128
        # self.down_encoder3_mask = self._make_layer(block, int((in_channels * 4) ), int((in_channels * 8) ), 1)  # 64  128
        # self.down_encoder4_mask = self._make_layer(block, (in_channels * 8) , (in_channels * 8) , 1)  # 64  128
    ...
    def forward(self, x, m): # m [bs, 16, 256, 256]
        ...
        ### Mask编码器，采用掩膜通道分离，顺序输入到编码器中
        # Mask Encoder
        m1s = self.inc_mask(m[:,0:1,:,:])         # 2 128 128
        m2s = self.down_encoder1_mask(self.pool(m1s))  # 4 128 128
        m3s = self.down_encoder2_mask(self.pool(m2s))  # 8 64  64
        m4s = self.down_encoder3_mask(self.pool(m3s))  # 16 32  32
        # m5s = self.down_encoder4(self.pool(m4s))  # 16 16  16
        for m_idx in range(1, m.shape[1]):
            m_sample = m[:,m_idx:m_idx+1,:,:]

            m1 = self.inc_mask(m_sample)
            m1s = torch.cat([m1s, m1],dim=1)        # 2*16 256 256
            m2 = self.down_encoder1_mask(self.pool(m1))
            m2s = torch.cat([m2s, m2], dim=1)       # 4*16 128 128
            m3 = self.down_encoder2_mask(self.pool(m2))
            m3s = torch.cat([m3s, m3], dim=1)       # 8*16 64  64
            m4 = self.down_encoder3_mask(self.pool(m3))
            m4s = torch.cat([m4s, m4], dim=1)       # 16*16 32  32
            # m5 = self.down_encoder4(self.pool(m4))
            # m5s = torch.cat([m5s, m5], dim=1)       # 256*16 16  16

        # #### Mask编码器，特征连接输入到编码器做融合
        # m1s = self.inc_mask(m) # 32 256 256
        # m2s = self.down_encoder1(self.pool(m1s))  # 64 128 128
        # m3s = self.down_encoder2(self.pool(m2s))  # 128 64  64
        # m4s = self.down_encoder3(self.pool(m3s))  # 256 32  32
        # # m5s = self.down_encoder4(self.pool(m4s))  # 256 16  16
        ...
        #  CCT
        x1, x2, x3, x4, att_weights = self.mtc(x1, x2, x3, x4, m1s, m2s, m3s, m4s)
...
if __name__ == '__main__':
    ...
    masks_set = torch.rand(1, 16, 256, 256)
    output = model(inputs, masks_set)
    flops, params = profile(model, (inputs,masks_set,))
