...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_loss(self, targets, pred, back, mask):
        ...
        l2_loss = (loss.sum(dim=(1, 2)) / mask.sum(dim=(1, 2))).sum() / mask.size(0)  # mean loss on removed patches
    ...
    def forward(self, imgs, targets, backs, masks):
        ...
        pred = self.forward_decoder(imgs)  # [N, L, p*p]
...
def mae_vit_base_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        img_size=16,
        patch_size=2, in_chans=1, embed_dim=512, depth=8, num_heads=16,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
...
#     model = MaskedAutoencoderViT(
#         img_size=128,
#         patch_size=16, in_chans=1, embed_dim=512, depth=8, num_heads=16,
#         decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
#         mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
#     return model
