...
class Node(nn.Module):
    def __init__(self, input_channels,  output_channels, noise_level_channel, norm_groups, dropout, num_blocks=1):
        super().__init__()
        self.layers = []
...
class UNet(nn.Module):
    def __init__(
        ...
        self.conv4_0 = Node(nb_filter[3], nb_filter[4], noise_level_channel, norm_groups, dropout, res_blocks)
        if len(self.nb_filter) == 6:
            self.conv5_0 = Node(nb_filter[4], nb_filter[5], noise_level_channel, norm_groups, dropout, res_blocks)
        ...
        if len(self.nb_filter) == 6:
            self.conv4_1 = Node(nb_filter[4] + nb_filter[5] + nb_filter[3], nb_filter[4], noise_level_channel, norm_groups, dropout, res_blocks)
        ...
        self.conv2_2 = Node(nb_filter[2] * 2 + nb_filter[3] + nb_filter[1], nb_filter[2],
                                        noise_level_channel, norm_groups, dropout, res_blocks)
        if len(self.nb_filter) == 6:
            self.conv3_2 = Node(nb_filter[3] * 2 + nb_filter[4] + nb_filter[2], nb_filter[3],
                                noise_level_channel, norm_groups, dropout, res_blocks)
        ...
        self.conv1_3 = Node(nb_filter[1] * 3 + nb_filter[2] + nb_filter[0], nb_filter[1],
                                        noise_level_channel, norm_groups, dropout, res_blocks)
        if len(self.nb_filter) == 6:
            self.conv2_3 = Node(nb_filter[2] * 3 + nb_filter[3] + nb_filter[1], nb_filter[2],
                                noise_level_channel, norm_groups, dropout, res_blocks)

        self.conv0_4 = Node(nb_filter[0] * 4 + nb_filter[1], nb_filter[0], noise_level_channel, norm_groups, dropout)
        if len(self.nb_filter) == 6:
            self.conv1_4 = Node(nb_filter[1] * 4 + nb_filter[2] + nb_filter[0], nb_filter[1], noise_level_channel, norm_groups,
                                dropout)

        if len(self.nb_filter) == 6:
            self.conv0_5 = Node(nb_filter[0] * 5 + nb_filter[1], nb_filter[0], noise_level_channel, norm_groups, dropout)

            self.conv0_5_final = Node(nb_filter[0] * 6, nb_filter[0], noise_level_channel, norm_groups, dropout)
        else:
        ...
        if self.deep_supervision:
            ...
            if len(self.nb_filter) == 6:
                self.final5 = nn.Conv2d(nb_filter[0], out_channel, kernel_size=1)
        else:
    ...
    def forward(self, x, time):
        ...
        if len(self.nb_filter) == 6:
            x5_0 = self.conv5_0(self.pool(x4_0), t)
            x4_1 = self.conv4_1(torch.cat([x4_0, self.up(x5_0), self.down(x3_1)], 1), t)
            x3_2 = self.conv3_2(torch.cat([x3_0, x3_1, self.up(x4_1), self.down(x2_2)], 1), t)
            x2_3 = self.conv2_3(torch.cat([x2_0, x2_1, x2_2, self.up(x3_2), self.down(x1_3)], 1), t)
            x1_4 = self.conv1_4(torch.cat([x1_0, x1_1, x1_2, x1_3, self.up(x2_3), self.down(x0_4)], 1), t)
            x0_5 = self.conv0_5(torch.cat([x0_0, x0_1, x0_2, x0_3, x0_4]), self.up(x1_4), 1, t)

        if len(self.nb_filter) == 6:
            Final_x0_5 = self.conv0_5_final(
                torch.cat([self.up_32(self.conv0_5_1x1(x5_0)), self.up_16(self.conv0_4_1x1(x4_1)), self.up_8(self.conv0_3_1x1(x3_2)),
                           self.up_4(self.conv0_2_1x1(x2_3)), self.up(self.conv0_1_1x1(x1_4)), x0_5], 1), t)
        else:
        ...
        if self.deep_supervision:
            ...
            if len(self.nb_filter) == 6:
                ...
                output5 = self.final4(Final_x0_5)
