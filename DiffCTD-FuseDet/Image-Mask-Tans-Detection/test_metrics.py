...
def cal_pd_fa(preds, labels, size):
    ...
    target = len(coord_label)  # 目标总数  直接就搞GT的连通域个数
    image_area_total = []  # 图像中预测的区域列表
    image_area_match = []
    distance_match = []
    ...
    dismatch = [x for x in image_area_total if x not in image_area_match]  # 在image里面 但是不在label里面

    dismatch_pixel = np.sum(dismatch)  # Fa 虚警个数 像素的虚警
    # print(self.dismatch_pixel)
    all_pixel = size[0] * size[1]
    PD = len(distance_match)

    return dismatch_pixel, all_pixel, PD, target
...
class ROCMetricPDFA():
    ...
    def __init__(self, nclass, bins):
        ...
        super(ROCMetricPDFA, self).__init__()
        self.nclass = nclass
        self.bins = bins
        self.dismatch_pixel_arr = np.zeros([11])
        self.all_pixel_arr = np.zeros([11])
        self.PD_arr = np.zeros([11])
        self.target_arr = np.zeros([11])
        # self.reset()
    ...
    def update(self, preds, labels, size):
        ...
        for iBin in range(self.bins + 1):
            ...
            dismatch_pixel, all_pixel, PD, target = cal_pd_fa(preds[0,0,:,:] > score_thresh, labels[0,0,:,:] > score_thresh, size)
            self.dismatch_pixel_arr[iBin] += dismatch_pixel
            self.all_pixel_arr[iBin] += all_pixel
            self.PD_arr[iBin] += PD
            self.target_arr[iBin] += target

    def get(self):
        Final_FA = self.dismatch_pixel_arr / self.all_pixel_arr  # tp_rates = recall = TP/(TP+FN)
        Final_PD = self.PD_arr / (self.target_arr + 0.001)  # fp_rates =  FP/(FP+TN)
